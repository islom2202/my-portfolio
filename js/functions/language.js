...
//fetch data
let data = null
fetch("data.json").then((res) => res.json()).then((content) => (data = content))

export const handleLang = (e) => {
  ...
  // contact-translation
  form_heading.textContent = data[e]["contact"]["contact-form"]["form-heading"]
  ...
  contact_list_item_heading.forEach(
    (tag, i) =>
      (tag.textContent = data[e]["contact"]["contact-list"][i]["item-heading"])
  )
